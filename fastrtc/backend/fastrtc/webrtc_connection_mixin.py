    ...
    async def handle_offer(self, body, set_outputs):
        ...
        if body.get("type") == "ice-candidate" and "candidate" in body:
            ...
            if webrtc_id not in self.pcs:
                # This is expected - ICE candidates can arrive after connection is closed
                logger.debug(
                    f"Received ICE candidate for unknown connection (likely already closed): {webrtc_id}"
                )
                return JSONResponse(
                    status_code=200,
                    content={
        ...
        def _(channel):
            ...
            def _(message):
                ...
                if channel.readyState == "open":
                    ...
                    # Route message to handler if it has handle_message method
                    for conn in self.connections.get(body["webrtc_id"], []):
                        if hasattr(conn.event_handler, 'handle_message'):
                            try:
                                conn.event_handler.handle_message(message)
                            except Exception as e:
                                logger.debug(f"Error in handler.handle_message: {e}")
